...
class DemoDisplay:
    def __init__(self):
        ...
        for series in sorted(os.listdir(os.path.join("images", "series"))):
            ...
            for image_file in os.listdir(os.path.join("images", "series", series)):
                images.append(
                    cv2.imread(os.path.join("images", "series", series, image_file))
                )
        ...
        cv2.putText(
            self.action_panel_title,
            "ACTION CLASSES",
            (40, 25),
            cv2.FONT_HERSHEY_SIMPLEX,
            0.65,
            self.action_panel_fg_color,
            2,
        )
    ...
    def convert_to_system_action_prediction(self, jester_prediction, jester_labels):
        ...
        # Series Navigation
        system_predictions[
            self.action_classes.index("Previous Series")
        ] = jester_prediction[jester_labels.index("Swiping Up")]
        system_predictions[
            self.action_classes.index("Next Series")
        ] = jester_prediction[jester_labels.index("Swiping Down")]

        # Series Image Navigation
        system_predictions[
            self.action_classes.index("Previous Image")
        ] = jester_prediction[jester_labels.index("Swiping Left")]
        system_predictions[self.action_classes.index("Next Image")] = jester_prediction[
            jester_labels.index("Swiping Right")
        ]

        # Browse Within Image
        system_predictions[self.action_classes.index("Browse Up")] = jester_prediction[
            jester_labels.index("Sliding Two Fingers Up")
        ]
        system_predictions[
            self.action_classes.index("Browse Down")
        ] = jester_prediction[jester_labels.index("Sliding Two Fingers Down")]
        system_predictions[
            self.action_classes.index("Browse Left")
        ] = jester_prediction[jester_labels.index("Sliding Two Fingers Left")]
        system_predictions[
            self.action_classes.index("Browse Right")
        ] = jester_prediction[jester_labels.index("Sliding Two Fingers Right")]

        # Play functionalities
        system_predictions[
            self.action_classes.index("Play In Reverse")
        ] = jester_prediction[jester_labels.index("Rolling Hand Backward")]
        system_predictions[
            self.action_classes.index("Play Forward")
        ] = jester_prediction[jester_labels.index("Rolling Hand Forward")]
        system_predictions[self.action_classes.index("Stop")] = (
            jester_prediction[jester_labels.index("Stop Sign")]
            + jester_prediction[jester_labels.index("Pushing Hand Away")]
        )

        # Brightness Manipulation
        system_predictions[
            self.action_classes.index("Increase Brightness")
        ] = jester_prediction[jester_labels.index("Pushing Two Fingers Away")]
        system_predictions[
            self.action_classes.index("Decrease Brightness")
        ] = jester_prediction[jester_labels.index("Pulling Two Fingers In")]

        # Rotational Manipulation
        system_predictions[
            self.action_classes.index("Rotate Counter-clockwise")
        ] = jester_prediction[jester_labels.index("Turning Hand Counterclockwise")]
        system_predictions[
            self.action_classes.index("Rotate Clockwise")
        ] = jester_prediction[jester_labels.index("Turning Hand Clockwise")]

        # Scale Manipilation
        system_predictions[self.action_classes.index("Zoom In")] = (
            jester_prediction[jester_labels.index("Zooming In With Full Hand")]
            + jester_prediction[jester_labels.index("Zooming In With Two Fingers")]
        )
        system_predictions[self.action_classes.index("Zoom Out")] = (
            jester_prediction[jester_labels.index("Zooming Out With Full Hand")]
            + jester_prediction[jester_labels.index("Zooming Out With Two Fingers")]
        )

        # Locking Mechanism
        system_predictions[
            self.action_classes.index("Thumb Up (Unlock)")
        ] = jester_prediction[jester_labels.index("Thumb Up")]
        system_predictions[
            self.action_classes.index("Thumb Down (Lock)")
        ] = jester_prediction[jester_labels.index("Thumb Down")]

        # No System Action
        system_predictions[self.action_classes.index("No System Action")] = (
            jester_prediction[jester_labels.index("Doing other things")]
            + jester_prediction[jester_labels.index("No gesture")]
            + jester_prediction[jester_labels.index("Drumming Fingers")]
            + jester_prediction[jester_labels.index("Pulling Hand In")]
            + jester_prediction[jester_labels.index("Pushing Hand Away")]
    ...
    def construct_action_panel(self, prediction):
        ...
        img = np.hstack(
            [np.vstack(self.action_labels), np.vstack(ACTION_PROBABILITIES)]
        )
    ...
    def construct_center_panel(self):
        ...
        if self.mode == "play":
            self.current_image = (self.current_image + 1) % len(
                self.series[self.current_series]["images"]
            )
        ...
        center_panel_x = (
            int((center_panel.shape[0] - current_image.shape[0]) / 2) + self.offset_x
        )
        center_panel_y = (
            int((center_panel.shape[1] - current_image.shape[1]) / 2) + self.offset_y
        )
        ...
        # current image within bounds
            ...
            center_panel[
                center_panel_x : center_panel_x + len_x,
                center_panel_y : center_panel_y + len_y,
            ] = current_image[
                current_image_x : current_image_x + len_x,
        ...
        cv2.putText(
            center_panel,
            self.series[self.current_series]["name"].upper(),
            (10, 20),
            cv2.FONT_HERSHEY_SIMPLEX,
            0.5,
            annot_text_color,
        ...
        cv2.putText(
            center_panel,
            "IMAGE " + str(self.current_image + 1),
            (10, 40),
            cv2.FONT_HERSHEY_SIMPLEX,
            0.5,
            annot_text_color,
        ...
        cv2.putText(
            center_panel,
            "BRIGHTNESS: " + str(self.brightness_level),
            (10, 960),
            cv2.FONT_HERSHEY_SIMPLEX,
            0.5,
            annot_text_color,
    ...
    def construct_navigation_panel(self):
        ...
        for i, series in enumerate(self.series):
            ...
            textsize = cv2.getTextSize(
                series["name"], cv2.FONT_HERSHEY_SIMPLEX, 0.5, 2
            )[0]
            ...
            textsize = cv2.getTextSize(
                images_cnt_text, cv2.FONT_HERSHEY_SIMPLEX, 0.5, 2
            )[0]
            text_x = int((series_item.shape[1] - textsize[0]) / 2)
            cv2.putText(
                series_item,
        ...
        navigation_panel = np.vstack(
            [self.status_image, navigation_label, series_list, blank]
        )
    ...
    def update_system_status(self, prediction):
        ...
        if self.current_action_counter >= self.action_threshold:
            ...
            # Unlock Application
            if self.status == "LOCKED" and action == self.action_classes.index(
                "Thumb Up (Unlock)"
            ):
            ...
            if self.status == "UNLOCKED":
                ...
                # Series Navigation
                    ...
                    action == self.action_classes.index("Previous Series")
                    and self.mode == "view"
                ):
                    self.current_series = (self.current_series - 1) % len(self.series)
                    self.current_image = 0
                elif (
                    action == self.action_classes.index("Next Series")
                    and self.mode == "view"
                ):
                    self.current_series = (self.current_series + 1) % len(self.series)
                    self.current_image = 0
                    ...
                    action == self.action_classes.index("Previous Image")
                    and self.mode == "view"
                ):
                    self.current_image = (self.current_image - 1) % len(
                        self.series[self.current_series]["images"]
                    ...
                    action == self.action_classes.index("Next Image")
                    and self.mode == "view"
                ):
                    self.current_image = (self.current_image + 1) % len(
                        self.series[self.current_series]["images"]
                ...
                # Play functionality
                    ...
                    action == self.action_classes.index("Play Forward")
                    and self.mode != "play"
                ):
                    self.mode = "play"
                elif (
                    action == self.action_classes.index("Play In Reverse")
                    and self.mode != "reverse"
                ):
                    self.mode = "reverse"
                elif (
                    action == self.action_classes.index("Stop") and self.mode != "view"
                ):
                    self.mode = "view"
    ...
    def get_gui_components(self, prediction, jester_labels):
        ...
        system_prediction = self.convert_to_system_action_prediction(
            prediction, jester_labels
        )
