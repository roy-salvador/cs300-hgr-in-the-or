...
# capture video from regular camera
...
while True:
    ...
    if len(current_frames) == demo_dnn.depth:
        ...
        print(
            "Network prediction took "
            + str(int(pipeline_processing_time.total_seconds() * 1000))
            + " milliseconds."
        )

        pipeline_start = datetime.datetime.now()
        action_panel, center_panel, navigation_panel = demo_display.get_gui_components(
            result, demo_dnn.jester_action_classes
        )
    else:
        pipeline_start = datetime.datetime.now()
        action_panel, center_panel, navigation_panel = demo_display.get_gui_components(
            np.zeros(demo_dnn.jester_num_classes), demo_dnn.jester_action_classes
        )
    ...
    cv2.imshow(
        "Medical Image Navigator Demo",
        np.hstack([navigation_panel, center_panel, right_panel]),
    )
    pipeline_end = datetime.datetime.now()
    pipeline_processing_time = pipeline_end - pipeline_start
    print(
        "Rendering application took "
        + str(int(pipeline_processing_time.total_seconds() * 1000))
        + " milliseconds."
    )
